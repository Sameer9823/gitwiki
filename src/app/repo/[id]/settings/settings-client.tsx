...
export function SettingsClient({ repositoryId, fullName, displayName, defaultBranch, status }: SettingsClientProps) {
  ...
  const reindex = async () => {
    ...
    try {
      ...
      showToast("Re-index started — only changed files will be reprocessed", "success");
      router.refresh();
    } catch (e) {
  ...
  return (
    <div className="max-w-2xl space-y-6">
      ...
      <div className="space-y-6 rounded-xl border border-border bg-surface p-5">
        ...
        <SettingsSection title="Indexing" description="Diffs against the last indexed commit and only reprocesses changed files. Falls back to a full index automatically if this repo hasn't been indexed before.">
