...
async function handler(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Incremental by default — reindexRepo.ts diffs against the last COMPLETED
  // snapshot and falls back to a full index itself if there isn't one yet.
  await inngest.send({
    name: "repo/reindex.requested",
    data: {
