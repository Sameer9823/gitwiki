...
export const generateWiki = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const pageResults: { slug: string; revised: boolean; issueCount: number }[] = [];
    ...
    for (const page of plannedPages) {
      ...
      const result = await step.run(`write-page-${page.slug}`, async () => {
        ...
        if (written.critiqueIssues.length > 0) {
          console.info(
            `[generateWiki] "${written.slug}" — critic flagged ${written.critiqueIssues.length} issue(s)${written.revised ? ", revised" : ""}: ${written.critiqueIssues.join("; ")}`
          );
        }

        return { slug: written.slug, revised: written.revised, issueCount: written.critiqueIssues.length };
      });
      pageResults.push(result);
    }

    return { repo: repoKey, pagesGenerated: plannedPages.map((p) => p.slug), pageResults };
  },
);
