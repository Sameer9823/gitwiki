...
export const indexRepo = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    try {
      ...
      if (fetched.filesTruncated || fetched.treeTruncated) {
        console.warn(
          `[indexRepo] ${repoKey}: indexing truncated (filesTruncated=${fetched.filesTruncated}, treeTruncated=${fetched.treeTruncated}) — only ${files.length} files were indexed. Raise MAX_INDEX_FILES or pass a higher maxFiles to fetchRepoFiles if this repo needs full coverage.`
        );
      }
      ...
      // Existing wiki pages (from a prior snapshot) may now be one more
      // reindex behind — recompute their freshness against the new snapshot
      // history. No-ops cheaply for a first-time index with no pages yet.
      await step.run("recalculate-wiki-freshness", async () => {
      ...
      return {
        ...
        treeTruncated: fetched.treeTruncated,
      };
    } catch (err) {
