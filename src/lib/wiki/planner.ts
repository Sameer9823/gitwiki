...
export async function planWikiPages(repositoryId: string, snapshotId: string): Promise<PlannedPage[]> {
  ...
  const hasGoManifest = pathsMatch(paths, /(^|\/)go\.mod$/);
  const hasRustManifest = pathsMatch(paths, /(^|\/)Cargo\.toml$/);
  const hasJavaManifest = pathsMatch(paths, /(^|\/)(pom\.xml|build\.gradle(\.kts)?)$/);
  ...
  if (hasPackageJson || hasPythonManifest || hasGoManifest || hasRustManifest || hasJavaManifest) {
    pages.push({
