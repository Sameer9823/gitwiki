...
export async function createIncrementalSnapshot(
  ...
  const newSnapshot = await prisma.repositorySnapshot.create({
    data: {
      repositoryId,
      branch: "unknown",
      status: "RUNNING",
  ...
  // 8. Degrade freshness of affected wiki pages
  if (affectedWikiPages.length > 0) {
    await prisma.wikiPage.updateMany({
      where: { repositoryId, slug: { in: affectedWikiPages } },
      data: { freshness: { decrement: 0.15 } },
    });
  }
