...
| **Indexing pipeline** (`indexRepo`) | Octokit `getTree` → skip binaries/lockfiles/large files → up to `MAX_INDEX_FILES` blobs (concurrent fetch, default 800) → symbol extraction (Babel AST for JS/TS; tree-sitter for Python/Go/Rust/Java: functions/classes/methods/interfaces/routes) → symbol-aware chunking → embed (`text-embedding-3-small`) → Pinecone `saveChunks` → persist `RepositoryFile`/`RepositorySymbol` linked to snapshot. Manual "Reindex now" runs incrementally against the last snapshot (`reindexRepo`), falling back to a full index if there isn't one yet. |
...
| **Living Wiki** | `wiki/planner.ts` proposes only pages with material in the snapshot (JS/Python/Go/Rust/Java manifests all recognized); `wiki/writer.ts` writes each page from hybrid context (wider `topK`) with guardrails not to invent, then `wiki/critic.ts` reviews the draft against the same sources for unsupported claims and gaps — the writer gets one revision pass if issues are found (toggle: `WIKI_CRITIC_ENABLED`). `generateWiki` Inngest function writes pages independently (retryable). Freshness starts at `1.0` and decays based on how many snapshots a page has drifted behind (`wiki/freshness.ts`), recalculated after every index and on a daily cron safety net — not just pages the dependency graph flags as directly affected. UI: `/repo/[id]/wiki` + `/wiki/[slug]` (markdown, TOC, freshness badge, sources). |
...
| **Change Intelligence** | `/repo/[id]/changes` — `ChangeReport` + `Commit` anchored diffs. `processPush` runs on GitHub push webhooks. `reviewPullRequest` runs on PR opened/reopened/synchronize webhooks — fetches the diff, runs impact analysis against the indexed import graph, and posts an AI review as a PR comment (toggle with `PR_REVIEW_ENABLED`). |
...
| `MAX_INDEX_FILES` | Optional. Max files indexed per repo (default `800`, up from a previously hardcoded `200`). Blob fetches run with bounded concurrency (`8` in flight) regardless of this value. When exceeded, `fetchRepoFiles` reports `filesTruncated: true` and `indexRepo` logs a warning — see `src/lib/github.ts`. |
| `GITHUB_WEBHOOK_SECRET` | Required for push-triggered incremental indexing and PR review. Set on the GitHub webhook (repo Settings → Webhooks), which must also be subscribed to "Pushes" and "Pull requests" events — see `src/app/api/webhooks/github/route.ts`. |
| `PR_REVIEW_ENABLED` | Optional. Set to `false` to stop posting automated review comments on PRs while leaving push-triggered indexing on. Defaults to enabled. See `src/lib/prReview.ts`. |
| `WIKI_CRITIC_ENABLED` | Optional. Set to `false` to skip the wiki critic's review/revision pass (see `src/lib/wiki/critic.ts`) and publish the writer's first draft directly. Defaults to enabled. |
